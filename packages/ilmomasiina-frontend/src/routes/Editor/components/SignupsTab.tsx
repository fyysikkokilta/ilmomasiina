...
const SignupsTab = () => {
  ...
  const signupsByQuota = useMemo(() => event && getSignupsByQuotaForAdminList(event, emails), [event, emails]);
  const csvSignups = useConvertSignupsToCSV(event, signups, emails.length > 0);
